...
# Added env vars for local build 
# Experimental, especific for the latest llvm toolchain
# and build with conda-forge env 
# =====
export CC="clang"
export CXX="clang++"
...
cmake -G$LFORTRAN_CMAKE_GENERATOR -DCMAKE_VERBOSE_MAKEFILE=ON -DWITH_LSP=yes -DWITH_LLVM=yes -DWITH_XEUS=yes -DCMAKE_PREFIX_PATH=$CONDA_PREFIX -DCMAKE_INSTALL_PREFIX=$CONDA_PREFIX -DCMAKE_INSTALL_LIBDIR="$CONDA_PREFIX/lib" -DCMAKE_BUILD_TYPE=$BUILD_TYPE -DWITH_RUNTIME_STACKTRACE=$ENABLE_RUNTIME_STACKTRACE -DWITH_INTERNAL_ALLOC_CHECK=yes -DUSE_DYNAMIC_ZSTD=no -DCMAKE_EXPORT_COMPILE_COMMANDS=OFF -DCMAKE_C_FLAGS="${CFLAGS}" -DCMAKE_CXX_FLAGS="${CXXFLAGS}" ..
